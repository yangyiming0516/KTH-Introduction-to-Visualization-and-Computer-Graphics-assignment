...
{
	double vRotate = 50;
	double innerR= 3, outerR = 6;
	int times = 10;
	double pertime = 360.0 / times;
	gDice[5]->modelMatrix().translate(-3, -3, 0);
	gDice[5]->modelMatrix().rotate(vRotate*gLastFrameTime,vl::fvec3(0, 0, 1));
	double nx = gDice[5]->getPosition().x();
	double ny = gDice[5]->getPosition().y();
	double L = sqrt(nx*nx+ny*ny);
	gDice[5]->modelMatrix().translate(-nx, -ny, 0);
	gDice[5]->modelMatrix().rotate(-vRotate*gLastFrameTime, vl::fvec3(0, 0, 1));
	gDice[5]->modelMatrix().translate(3, 3 , 0);
	
	double dR = asin(ny / sqrt(nx*nx + ny*ny))/3.14159265*180;
	if (nx < 0) dR = 180-dR;
	if (dR < 0) dR += 360;
	double xR = dR - floor(dR / pertime) * pertime;
	double percent = abs(pertime/2 - xR) / (pertime/2);
	gDice[5]->modelMatrix().translate(((outerR - innerR)*percent + innerR)*nx / L, ((outerR - innerR)*percent + innerR)*ny / L, 0);
