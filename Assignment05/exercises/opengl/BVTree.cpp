...
const std::vector<int>& BVTree::intersectBoundingBoxes(const Ray &ray, const float maxLambda) const
{
#if defined(_OPENMP)
...
void BVTree::createNodes(const std::vector<Vec3f> &vertexPositions,
    const std::vector<Vec3i> &triangleIndices)
{
  ...
  {
    const Vec3f &v0 = vertexPositions[triangleIndices[i][0]];
    const Vec3f &v1 = vertexPositions[triangleIndices[i][1]];
    const Vec3f &v2 = vertexPositions[triangleIndices[i][2]];
...
void BVTree::build(const std::vector<Vec3f> &vertexPositions,const std::vector<Vec3i> &triangleIndices)
{
  ...
  std::vector<Vec3f>().swap(mTempAreasLeft);
  std::vector<Vec3f>().swap(mTempAreasRight);
...
//  std::cerr<<"Num Triangles: "<<numTriangles<<std::endl;
  ...
  int splitDimension=3;
  size_t splitIndex=0;
  float minCosts=std::numeric_limits<float>::max();

  //compute total area
  float totalArea=mNodes[rootNodeIndex].bbox.computeArea();  
  ...
  for(int dim=0;dim<3;++dim)
  {
    ...
    {
      float leftArea = mTempAreasLeft[i+offset][dim];
      float rightArea = mTempAreasRight[i+offset][dim];
      float splitCosts = (leftArea*(i+1)+rightArea*(numTriangles-1-i))/totalArea;
