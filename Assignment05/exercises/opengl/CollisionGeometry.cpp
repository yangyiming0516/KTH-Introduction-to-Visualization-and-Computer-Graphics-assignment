...
CollisionGeometry::CollisionGeometry() : mInitialized(false)
{
  ...
  mShininess = 10.f;
  mLineWidth = 0.f;
  mColor = Vec4f(0.5,0.5,0.5,1);
  mLineColor = Vec4f(0,0,0,0);
  mLightPosition[0]=Vec4f(0,0,0,0);
  mLightPosition[1]=Vec4f(0,0,0,0);
  mLightPosition[2]=Vec4f(0,0,0,0);
}
...
void CollisionGeometry::init(const std::vector<Vec3f>& p, const std::vector<Vec3f>& n, const std::vector<unsigned int>& t)
{
  this->clear();
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(Vec3f) * p.size(), &p[0], GL_STATIC_DRAW);
  ogl::printOpenGLError();

  // Create and copy index buffer on GPU
  glGenBuffers(1, &mNormalBuffer);
  glBindBuffer(GL_ARRAY_BUFFER, mNormalBuffer);
  glBufferData(GL_ARRAY_BUFFER, sizeof(Vec3f) * n.size(), &n[0], GL_STATIC_DRAW);
  ogl::printOpenGLError();
...
void CollisionGeometry::setMaterial(float shininess, const Vec3f& color, float lineWidth, const Vec3f& lineColor)
{
  mShininess=shininess;
  mColor=Vec4f(color,1);
  mLineWidth=lineWidth;
  mLineColor=Vec4f(lineColor,1);
}

Vec3f CollisionGeometry::getPosition() const
{
  return mModelMatrix*Vec3f(0,0,0);
}
...
void CollisionGeometry::clear()
{
  ...
  if(!mInstance)
  {
    std::vector<Vec3f>().swap(mCollisionPositions);
...
bool
  CollisionGeometry::closestIntersection(const Ray &ray, float maxLambda, RayIntersection& intersection) const
{
